...
var fmt_url_map = args['fmt_url_map'].split(',');
for (var i = 0; i < fmt_url_map.length; i++) {
  pair = decodeURIComponent(fmt_url_map[i]).split('|')
  for (var j = 0; j < formatsAvailable.length; j++) {
    if (formatsAvailable[j] == pair[0]) {
      urlsAvailable[j] = pair[1] + "&t=" + t;
    }
  }
}
